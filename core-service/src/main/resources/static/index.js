angular.module('app', ['ngStorage']).controller('IndexController', function ($scope, $http, $localStorage){
    ...
    $scope.removeFromCart = function (product_id) {
        $http.get('http://localhost:8190/store-carts/api/v1/cart/remove/' + product_id).then(function (response) {
            $scope.loadCart();
        });
    }

    $scope.addToCart = function (product_id) {
        $http.get('http://localhost:8190/store-carts/api/v1/cart/add/' + product_id).then(function (response) {
            $scope.loadCart();
        });
    }

    $scope.loadCart = function () {
        $http.get('http://localhost:8190/store-carts/api/v1/cart').then(function (response) {
            $scope.cart = response.data;
        });
    }

    $scope.clearCart = function (){
        $http.get('http://localhost:8190/store-carts/api/v1/cart/clear').then(function (response) {
            $scope.loadCart();
        });
    }

    $scope.removeItemFromCart = function (product_id) {
        $http.get('http://localhost:8190/store-carts/api/v1/cart/delete/' + product_id).then(function (response) {
            $scope.loadCart();
        });
    }
